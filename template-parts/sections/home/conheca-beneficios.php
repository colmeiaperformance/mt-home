<section class="conheca-beneficios">
  <div class="container">
    <h2>Conheça os <strong>benefícios</strong> da MT</h2>
    <div class="row align-items-center justify-content-center">
      <div class="col-12 col-lg-3 d-flex align-items-center justify-content-center p-0">
        <div class="box rounded-circle text-center">
          <div class="default">
              <img class="img-fluid" src=<?php echo get_template_directory_uri() . '/images/beneficios/mental.png' ?>
            alt="Benefícios: Saúde Mental">
              <h4>Saúde Mental</h4>
              <span>+</span>
          </div>
           <div class="hover mt-3">
            <p>
              Reduz ansiedade, depressão, síndrome do pânico e  burnout. Promove paz interior, autoconfiança e felicidade.
            </p>
            
          </div>
        </div>
      </div>
      <div class="col-12 col-lg-3 d-flex align-items-center justify-content-center p-0">
         <div class="box rounded-circle text-center">
          <div class="default">
              <img class="img-fluid" src=<?php echo get_template_directory_uri() . '/images/beneficios/fisica.png' ?>
              alt="Benefícios: Saúde Mental">
            <h4>Saúde Física</h4>
            <span>+</span>
          </div>
           <div class="hover mt-3">
            <p>
              Melhora a qualidade do sono, a frequência cardíaca, a pressão arterial e outros importantes indicadores de saúde.
            </p>
            
          </div>
        </div>
      </div>
      <div class="col-12 col-lg-3 d-flex align-items-center justify-content-center p-0">
         <div class="box rounded-circle text-center">
          <div class="default">
            <img class="img-fluid" src=<?php echo get_template_directory_uri() . '/images/beneficios/emocional.png' ?>
              alt="Benefícios: Saúde Mental">
            <h4>Inteligência emocional</h4>
            <span>+</span>
          </div>
          <div class="hover mt-3">
            <p>
              Promove autoconhecimento, empatia, melhora a qualidade dos relacionamentos e leva à autoreealização.
            </p>
            
          </div>
        </div>
      </div>
      <div class="col-12 col-lg-3 d-flex align-items-center justify-content-center p-0">
         <div class="box box-last rounded-circle text-center">
          <div class="default">
              <img class="img-fluid" src=<?php echo get_template_directory_uri() . '/images/beneficios/potencia.png' ?>
            alt="Benefícios: Saúde Mental">
            <h4>Potencial Mental</h4>
            <span>+</span>
            </div>
             <div class="hover mt-3">
            <p>
              Aumenta o foco, a criatividade e a clareza mental. Melhora a memória, a percepção e a capacidade de analisar e resolver problemas.
            </p>
            
          </div>
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col-12 quote">
        <div>
          <p>O propósito da vida é a <strong>expansão da felicidade</strong></p>
          <h4>Maharishi Mahesh Yogi</h4>
        </div>
    </div>
    </div>
  </div>
</section>